...
export const ModalCallNotification: React.FC = () => {
    ...
    return (
        <Dialog open={isReceivingCall} onOpenChange={() => rejectCall()}>
            <DialogContent autoFocus={false} className='h-auto max-w-md rounded-xl bg-gradient-to-b from-blue-50 to-white p-6 shadow-xl'>
                <DialogHeader>
                    <DialogTitle className='flex flex-col items-center'>
                        <div className='relative mb-2'>
                            {/* Vòng tròn hiệu ứng rung chuông */}
                            <div className={`absolute inset-0 rounded-full ${ringEffect ? 'bg-blue-100 scale-125' : 'bg-transparent scale-150'} transition-all duration-500 ease-in-out`} />
                            <div className={`absolute inset-0 rounded-full ${ringEffect ? 'bg-blue-50 scale-110' : 'bg-transparent scale-125'} transition-all duration-500 ease-in-out`} />
                ...
                <div className='mt-6 flex items-center justify-center gap-8'>
                    <Button
                        ...
                        size='icon'
                        onClick={rejectCall}
                        className='flex h-16 w-16 items-center justify-center rounded-full bg-red-500 shadow-lg hover:bg-red-600 active:scale-95 transition-transform'
                    >
                    ...
                    <Button
                        ...
                        size='icon'
                        onClick={answerCall}
                        className='flex h-16 w-16 items-center justify-center rounded-full bg-green-500 shadow-lg hover:bg-green-600 active:scale-95 transition-transform'
                    >
